export class CheckLCPImageBeforeSave extends $e.modules.hookData.Dependency {
	...
	async apply() {
		...
		const webVitals = await this.getWebVitalsFromPreview( deviceMode );
		const lcpImages = webVitals.lcp.entries.filter( ( entry ) => 'IMG' === entry.nodeName );
	...
	async getWebVitalsFromPreview( device ) {
		return await new Promise( ( resolve, reject ) => {
			...
			const eventRef = window.addEventListener( 'message', ( event ) => {
				if ( event.data?.name && 'elementor/web-vitals/editor' == event.data?.name ) {
